class Verse {
	...
	*/
	...
	try_template(syllables) {
        ...
        var temp_str = this.template.template;
		var syllables = this.syllables;
		
        syllables -= this.template.syllables;
        var syl_list = new Array(this.template.substitutions).fill(1);
        ...
        while (temp_str.length > 0) {
            if (! temp_str.includes("<")) {
				...
				var word = this.dictionary.get_random_word(pos.toLowerCase(), form, syl_list[syl_index]);
                syl_index += 1;
                if (! word) {
                    return false;
                }
